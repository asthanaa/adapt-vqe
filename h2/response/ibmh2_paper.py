...
def test():
    ...
    #print('final total energies ', final_total_energies)
    #print('W',W)
    ...
    ''' 
    Calculating <0|n> and <n|n> using qEOM

    '''
    print('type(aval[0]):', type(aval[0]))
    mat_dim = 2 * len(op)
    overlap_n_0 = []
    for n in range(mat_dim):
        ...
        for i in range(mat_dim):        
            ...
            if i < int(mat_dim/2):
                mat1 = op[i]*coeff_i
                overlap += qeom.expvalue(v.transpose().conj(),mat1,v)[0,0]
            else:
                mat1 = op[i- len(op)].transpose().conj()*coeff_i
                overlap += qeom.expvalue(v.transpose().conj(),mat1,v)[0,0]
        overlap_n_0.append(overlap)
    print('overlap_n_0: ', overlap_n_0) 

    '''
    O_dagger_j = []
    for j in range(len(op)):
        #O_dagger_j = op[j].transpose().conj().dot(v)
        O_dagger_j = op[j].transpose().conj()
        expect = qeom.expvalue(v.transpose().conj(),O_dagger_j,v)[0,0]
        #print('O_dagger_j: ', O_dagger_j)
        #print('expect: ', expect)
   '''
     
    for i in range(len(op)):
        mat=qeom.comm2(hamiltonian, op[i]).dot(-1.0 *op[i].transpose().conj())
        expect = qeom.expvalue(v.transpose().conj(),mat,v)[0,0]
        print(expect)
