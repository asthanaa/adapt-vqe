...
def adapt_vqe_me(hamiltonian_op, pool, reference_ket,
        adapt_conver    = 'norm',
        adapt_thresh    = 1e-3,
        theta_thresh    = 1e-7,
        adapt_maxiter   = 100,
...
# {{{
    ...
    e_curr_ad = np.zeros(100)
    e_curr_me2 = np.zeros(100)
    e_curr_me3 = np.zeros(100)
    e_curr_me4 = np.zeros(100)
    e_curr_me5 = np.zeros(100)
    ...
    for n_iter in range(0,adapt_maxiter):
        ...
        e_curr_ad[n_iter] = curr_state.T.conj().dot(sig)[0,0]
        e_curr_me2[n_iter]=me.me(2, hamiltonian.todense(),curr_state.todense())
        e_curr_me3[n_iter]=me.me(3, hamiltonian.todense(),curr_state.todense())
        e_curr_me4[n_iter]=me.me(4, hamiltonian.todense(),curr_state.todense())
        e_curr_me5[n_iter]=me.me(5, hamiltonian.todense(),curr_state.todense())
        ...
        print(var.real, np.sqrt(var.real), uncertainty, var.real)
        ...
        for si in range(len(ansatz_ops)):
            ...
            print(" %4i %12.8f %s" %(si, parameters[si], opstring) )
        #file.close()
    return trial_model.curr_energy, curr_state, n_iter, e_curr_ad,e_curr_me2, e_curr_me3, e_curr_me4
def adapt_vqe_edit2(hamiltonian_op, pool, reference_ket,
        adapt_conver    = 'norm',
        adapt_thresh    = 1e-3,
        theta_thresh    = 1e-7,
        adapt_maxiter   = 200,
