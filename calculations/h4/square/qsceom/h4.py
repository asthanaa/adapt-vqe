...
def test():
   for h in range(11):
        ...
        ee=qeom.sceom(op,barH,reference_ket,e)
        original_stdout = sys.stdout

        print(st,repr(np.sort(ee.real)+e))
        print(np.sort(ee.real)+e)
        with open('states', 'a') as f:
            sys.stdout = f # Change the standard output to the file we created.
            print(st,e, ',',repr(np.sort(ee.real)+e))
            sys.stdout = original_stdout
        with open('ee', 'a') as f:
            sys.stdout = f # Change the standard output to the file we created.
            print(str(st),str(repr(np.sort(ee.real))))
            sys.stdout = original_stdout
