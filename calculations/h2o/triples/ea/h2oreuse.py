...
def test():
    ...
    ## list of everything to save
    #list_save = []
    #list_save.append(e)
    #list_save.append(v)
    #list_save.append(params)
    #list_save.append(ansatz_mat)
    #output = open('eigvec.dat', 'wb')
    #pickle.dump(list_save, output) # converts array to binary and writes to output
    output = open('eigvec.dat', 'rb')
    ...
    #print('op[0] is',op[0])
    opt=triplesqeom.createops_eat(n_orb,n_a,n_b,n_orb-n_a,n_orb-n_b,reference_ket)
    print('opt[0] is',opt[0])
    ...
    Hmat=np.zeros((len(op)+len(opt),len(op)+len(opt)))
    ...
    #        mat=qeom.comm3(opt[i].transpose().conj(),barH,opt[j])
    #        Hmat[len(op)+i,len(op)+j]=qeom.expvalue(reference_ket.transpose().conj(),mat,reference_ket)[0,0].real
