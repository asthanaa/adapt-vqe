...
def test():
    ...
    for i in range(len(op)):
        for j in range(len(op)):
            left=qeom.wfex(params,ansatz_mat,op[j],reference_ket)
            right=qeom.wfex(params,ansatz_mat,op[i],reference_ket)
            #left=scipy.sparse.csr_matrix(left)
            #right=scipy.sparse.csr_matrix(right)
            Hmat[i,j]=qeom.expvalue(left.transpose().conj(),hamiltonian,right)[0,0].real
